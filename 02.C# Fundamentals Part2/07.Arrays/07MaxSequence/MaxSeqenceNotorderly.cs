    ...
    class MaxIncreasingSequence
    {
        static void Main()
        {
            ...
            for (int x = 0; x < arr.Length; x++)
            {
                ...
                {
                    if ((arr[i] < arr[x]) && 
                        (len[i] + 1 > len[x])) // Правим проверка за по малък елемент
                    {
                        len[x] = len[i] + 1;
                    }
                }
            ...
            result = new int[len[index]];
            result[len[index] - 1] = arr[index]; // Добавяме най-големия елемент

            // Вървим назад по елементите и печатим по-малките
            for (; index >= 0; index--)
            {
                if ((arr[index] < arr[prevIndex]) && 
                    (len[prevIndex] == len[index] + 1))  // Проверяваме дали има по-малък елемент от последния намерен и дали дължните на последователностите съвпадат
                {
                    result[len[index]-1]=(arr[index]);
